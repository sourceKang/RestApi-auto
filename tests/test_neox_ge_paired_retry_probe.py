...
from services.neox_config.service import GE_ENABLE_ACCEPTED_PAYLOAD_FILE, GE_PAIRED_RETRY_CONFIG_FILE, vlan_payload
...
PAIRED_RETRY_CONFIG_FILE = GE_PAIRED_RETRY_CONFIG_FILE
...
def test_ge_paired_retry_probe_set_readwrite(
    ...
    ssh_username, ssh_password = neox_cli_credentials(env_config, "GE paired retry probe")

    retry_config = json.loads(PAIRED_RETRY_CONFIG_FILE.read_text(encoding="utf-8"))
    ...
    commands = [
        ...
        f"show interface ge {target.ge_slot_id}-{target.ge_port_id} fdb",
    ]
    ...
    if retry_config.get("preconditions", {}).get("create_vlan"):
    ...
    api_client.request("DELETE", neox_config_service.ge_path(), session=readwrite_session)
    write_paired_retry_report(neox_config_service, accepted_content, results, final_cli_outputs)
...
def record_result(
    ...
    *,
    ...
    return {
        ...
        "cli_basis": candidate.get("cli_basis"),
...
def write_paired_retry_report(
    ...
    reports_dir = root / "reports" / "paired-retry-probes"
    reports_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = reports_dir / f"neox_ge_paired_retry_probe_{timestamp}.json"
    target = neox_config_service.target()
    data = {
        "target": {
